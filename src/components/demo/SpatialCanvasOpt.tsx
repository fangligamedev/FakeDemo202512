...
import { DEMO_SCRIPT, DemoStep, AssetType } from '@/config/demo_script_opt';
...
export default function SpatialCanvasOpt() {
    ...
    const match3FeedbackV1 = "优化方向：改为可爱女孩角色 + 水果主题。";
    const match3FeedbackV2 = "本地化：应用中国汉服风格 & 中文界面。";
    ...
    const runBuilderSequence = async () => {
        ...
        for (let i = 0; i < builderSteps.length; i++) {
            ...
            // --- INJECTED FEEDBACK LOGIC ---
            if (step.id === 'step_v1_prototype') {
                await showFeedback(match3FeedbackV1);
            }
            if (step.id === 'step_v2_pivot') {
                await showFeedback(match3FeedbackV2);
            }
        }
    ...
    return (
        <div className="w-full h-screen bg-[#020202] text-white font-sans overflow-hidden relative user-select-none cursor-none">
            ...
            {showGamePrompt && <PromptOverlay text="生成一款当前流行的三消游戏 (V1 -> V2 -> V3 优化演示)" onComplete={handleGamePromptDone} />}
            {/* Feedback Overlay */}
            <AnimatePresence>
                {feedbackText && (
